...
- Phase 7.4 (controller + platform-client coverage uplift): every gate package clears the IMPL-0001 ≥80% bar. controller 84.2%, platform 100%, platform/forgejo 90.4%, platform/github 84.8%, sharding 92.0%, jobspec 92.7%, observability 88.1%. Lift came from fake.NewClientBuilder() tests covering the IO helpers (mirrorCredential, ensureShardConfigMap, ensureWorkerJob, observeJob, refreshActiveRuns, gcOldRuns, createRun, scansForPlatform, platformsForSecret), client-option tests for the platform SDKs, and `WithInterceptorFuncs`-driven Reconcile-wrapper tests for the status-conflict + status-update-error paths.
