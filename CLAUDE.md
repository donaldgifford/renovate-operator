...
Coverage today (per `make test-coverage`): controller package is the standout gap at 17.2% (target ≥80%). platform/github 70.3%, platform/forgejo 75.0% also under the bar. clock/conditions/credentials/jobspec/platform/sharding all clear the bar.

Phase 7 remaining: GitHub stub + Forgejo + parallelism e2e, controller-coverage uplift, `test/manual/README.md`. Phase 8 (CI/release), Phase 9 (homelab cutover) remain.
