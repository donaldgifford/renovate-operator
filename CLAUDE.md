...
- **Renovate v43+ GitHub App auth requires `autodiscover=true`** — **Phase 9 acceptance blocker** (surfaced after PodSecurity fix unblocked workers). Operator was setting `RENOVATE_PLATFORM=github` + `RENOVATE_GITHUB_APP_ID/KEY` + `RENOVATE_AUTODISCOVER=false` + `RENOVATE_REPOSITORIES`, expecting Renovate to mint App-installation tokens itself. v43+ only mints tokens along the autodiscover code path; with autodiscover=false + App credentials, platform init fails with `"You must configure a GitHub token"`. Tracked in [INV-0003](docs/investigation/0003-renovate-v43-github-app-auth-requires-autodiscover-not.md). Fix bifurcates env wiring by auth type: App auth → `RENOVATE_AUTODISCOVER=true` + `RENOVATE_AUTODISCOVER_FILTER=<shard's repos>` (set by entrypoint shell from the shard JSON); token auth keeps the `RENOVATE_REPOSITORIES + autodiscover=false` path. Sharding decision still owned by the operator. Bundled with INV-0001/INV-0002/PodSecurity for v0.1.2.
