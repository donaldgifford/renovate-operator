...
Kubernetes Operator for Renovate
...
```sh
make docker-build docker-push IMG=<some-registry>/kube:tag
```
...
```sh
make deploy IMG=<some-registry>/kube:tag
```
...
```sh
make build-installer IMG=<some-registry>/kube:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/kube/<tag or branch>/dist/install.yaml
```
...
1. See that a chart was generated under 'dist/chart', and users can obtain this
   solution from there.
